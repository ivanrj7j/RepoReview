...
def evaluateRepo(repoUrl: str):
    ...
    try:
        ...
        logging.info(f"Evaluation completed for {repo} in {elapsedTime} seconds.")
...
def evaluateWithLoading(repoUrl: str):
    ...
    def worker():
        try:
            ...
            logging.error("Unexpected error in background thread.", exc_info=True)
            result_container[0] = ("**Error:** Something went wrong. Please try again!", 0)
...
with gr.Blocks() as ui:
    ...
    gr.Markdown("Enter a GitHub repository URL to analyze its structure and quality.")
    
    repoInput = gr.Textbox(label="GitHub Repo URL", placeholder="https://github.com/user/repo", elem_id="repoInput")
    submitButton = gr.Button("Evaluate Repo", elem_id="submitButton")
    
    resultOutput = gr.Markdown()
    timerOutput = gr.Markdown()
    
    submitButton.click(evaluateWithLoading, inputs=repoInput, outputs=[resultOutput, timerOutput])
